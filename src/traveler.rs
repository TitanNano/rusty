    ...
    fn from(value: Ast::expression::ArrowBody) -> AstFunctionBody {
        match value {
            ...
            Ast::expression::ArrowBody::Expression(expression) => {
                AstFunctionBody::SingleExpression(**expression)
            }
        }
    }
}
...
pub fn travel_ast<'ast>(
    ast: Ast::StatementList<'ast>,
) -> Vec<AstEvent<'ast, ExpressionNodeStruct<'ast>>> {
...
pub fn travel_ast_statement<'ast>(
    statement: Ast::Statement<'ast>,
) -> Vec<AstEvent<'ast, ExpressionNodeStruct<'ast>>> {
    let mut event_record = vec![];
    ...
    match statement {
        ...
        Ast::Statement::If(if_statement) => {
            ...
            event_record.extend(local_event_record);
            event_record.push(AstEvent::ConsequentBody {
                test: test.clone(),
                statement: **if_statement.consequent,
            });

            if if_statement.alternate.is_some() {
                event_record.push(AstEvent::AlternateBody {
                    expression: test.clone(),
                });
            }

            event_record.push(AstEvent::AfterIf {
                expression: test.clone(),
            });
        }
...
pub fn travel_expression<'ast>(
    expression: Ast::ExpressionNode<'ast>,
) -> (
    ExpressionNodeStruct<'ast>,
    Vec<AstEvent<'ast, ExpressionNodeStruct<'ast>>>,
) {
    ...
    let expression = match expression.item {
        Ast::Expression::Binary(binary_expression) => {
            ...
            match operator {
                Ast::OperatorKind::Assign => {
                    event_record.push(AstEvent::Assignment {
                        node: node.clone(),
                        left,
                        right,
                    });
                ...
                Ast::OperatorKind::Addition => {
                    event_record.push(AstEvent::Addition {
                        node: node.clone(),
                        left,
                        right,
                    });
                ...
                Ast::OperatorKind::StrictEquality => {
                    event_record.push(AstEvent::Equality {
                        node: node.clone(),
                        left,
                        right,
                    });
        ...
        Ast::Expression::Conditional(conditional_expression) => {
            ...
            let (consequent, cons_event_record) =
                travel_expression(conditional_expression.consequent);
            let (alternate, alt_event_record) =
                travel_expression(conditional_expression.consequent);
            ...
            event_record.push(AstEvent::Conditional {
                node: node.clone(),
                test,
                consequent,
                alternate,
        ...
        Ast::Expression::Member(member_expression) => {
            ...
            event_record.push(AstEvent::PropertyAccess {
                node: node.clone(),
                object,
                property,
            });
        ...
        Ast::Expression::Identifier(identifier_expression) => {
            ...
            event_record.push(AstEvent::Identifier {
                node: node.clone(),
        ...
        Ast::Expression::Void => ExpressionNodeStruct::from(expression),

        Ast::Expression::Literal(literal_expression) => {
            let node = ExpressionNodeStruct::from(expression);

            event_record.push(AstEvent::Literal {
                node: node.clone(),
        ...
        Ast::Expression::Array(array_expression) => {
            ...
            event_record.push(AstEvent::Array {
                node: node.clone(),
        ...
        Ast::Expression::Sequence(sequence_expression) => {
            ...
            event_record.push(AstEvent::Sequence {
                node: node.clone(),
                sequence: sequence_expression.body,
            });
        ...
        Ast::Expression::This(this_expression) => {
            ...
            event_record.push(AstEvent::This {
                node: node.clone(),
        ...
        Ast::Expression::MetaProperty(meta_property) => {
            panic!(
                "got meta property access {}.{}",
                **meta_property.meta, **meta_property.property
            );
        }
        ...
        Ast::Expression::ComputedMember(computed_member) => {
            ...
            event_record.push(AstEvent::DynamicPropertyAccess {
                node: node.clone(),
                object,
                property,
            });
        ...
        Ast::Expression::Call(function_call) => {
            let node = ExpressionNodeStruct::from(expression);
            let (function, _local_event_record) = travel_expression(function_call.callee);
            ...
            event_record.push(AstEvent::FunctionCall {
                node: node.clone(),
                function,
                arguments,
            });
        ...
        Ast::Expression::Prefix(prefix) => {
            ...
            event_record.push(AstEvent::PreOrPostFix {
                node: node.clone(),
                operand,
                operator,
            });
        ...
        Ast::Expression::Spread(spread_expression) => {
            ...
            event_record.extend(local_event_record);
            event_record.push(AstEvent::Spread {
                node: node.clone(),
                argument,
            });
        ...
        Ast::Expression::Object(object_expression) => {
            ...
            event_record.push(AstEvent::Object {
                node: node.clone(),
        ...
        Ast::Expression::Arrow(arrow_function) => {
            ...
            event_record.push(AstEvent::Function {
                node: node.clone(),
                params,
                body: function_body,
            });
        ...
        Ast::Expression::Class(class_expression) => {
            ...
            event_record.push(AstEvent::Class {
                class_expression,
                node: node.clone(),
            });
